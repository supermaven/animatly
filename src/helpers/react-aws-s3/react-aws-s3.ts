...
import { dateYMD, xAmzDate } from "./Date.ts"
import { IConfig, DeleteResponse, UploadResponse } from "./types.ts"
import { throwError } from "./ErrorThrower.ts"
...
class ReactS3Client {
    ...
    public async deleteFile(fileName: string): Promise<DeleteResponse> {
      const url_host: string = GetUrl(this.config)
      const url: string = `${url_host}/${this.config.dirName ? this.config.dirName + "/" : ""}${fileName}`
